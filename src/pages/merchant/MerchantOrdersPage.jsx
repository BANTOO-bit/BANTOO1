...
function MerchantOrdersPage() {
    ...
    const confirmAcceptOrder = async () => {
        ...
        try {
            ...
            await orderService.updateStatus(selectedOrder.dbId, 'accepted', { prep_time: prepTime })
                    ...
                    {activeTab === 'baru' && currentOrders.length > 0 && (
                        <section className="mt-2 bg-blue-50 dark:bg-blue-900/10 rounded-2xl p-4 flex items-center gap-3 border border-blue-100 dark:border-blue-800/30">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-[10px] text-blue-600 dark:text-blue-400 leading-tight mt-0.5">Ada 12 driver di sekitar area warungmu siap menerima pesanan.</p>
                            </div>
                        </section>
                    )}
                </div>
                    ...
                    <div className="relative bg-white dark:bg-card-dark w-full max-w-[340px] rounded-[24px] p-6 shadow-2xl flex flex-col gap-6 transform transition-all scale-100 animate-fade-in">
                        <div className="text-center flex flex-col items-center gap-3">
                            <div className="w-12 h-12 rounded-full bg-orange-50 dark:bg-orange-900/20 flex items-center justify-center text-primary mb-1">
                                <span className="material-symbols-outlined text-[28px] fill" style={{ fontVariationSettings: "'FILL' 1, 'wght' 400" }}>inventory_2</span>
                            </div>
                            <h3 className="text-xl font-bold text-text-main dark:text-white">Serahkan ke Driver?</h3>
                            <p className="text-sm text-text-secondary dark:text-gray-400 leading-relaxed px-2">
                                Pastikan nomor pesanan <span className="font-bold text-text-main dark:text-white">#{selectedOrder?.id}</span> sesuai dengan yang ada pada aplikasi Driver.
                            </p>
                        </div>
