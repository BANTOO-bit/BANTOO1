...
function HelpPaymentPage({ onNavigate }) {
    ...
    return (
        <div className="relative min-h-screen flex flex-col bg-background-light pb-6">
            ...
            <header className="sticky top-0 z-20 bg-background-light/95 backdrop-blur-md px-4 pt-12 pb-4 flex items-center justify-between border-b border-transparent transition-colors">
                ...
                <h1 className="text-text-main text-lg font-bold tracking-tight absolute left-1/2 transform -translate-x-1/2">Pembayaran</h1>
            ...
            <main className="flex flex-col gap-6 px-4 pt-4">
                {/* FAQ List */}
                <section>
                    <h2 className="text-base font-bold text-text-main mb-4">Topik Populer</h2>
                    <div className="flex flex-col gap-3">
                        <HelpOptionButton label="Mengatasi Gagal Transaksi" onClick={() => onNavigate?.('helpTransactionFailed')} />
                        <HelpOptionButton label="Cara Isi Saldo" onClick={() => onNavigate?.('helpTopUp')} />
                        <HelpOptionButton label="Prosedur Refund Dana" onClick={() => onNavigate?.('helpRefund')} />
                        <HelpOptionButton label="Metode Pembayaran Tersedia" onClick={() => onNavigate?.('helpPaymentMethods')} />
                        <HelpOptionButton label="Kendala Voucher & Promo" onClick={() => onNavigate?.('helpVoucher')} />
                    </div>
                </section>
