...
function HelpOrderPage({ onNavigate }) {
    const navigate = useNavigate()
    const handleQuestionClick = (title) => {
        onNavigate?.('helpDetail', {
            category: {
                title: title,
                from: 'helpOrder'
            }
    ...
    return (
        <div className="relative min-h-screen flex flex-col bg-background-light pb-6">
            ...
            <header className="sticky top-0 z-20 bg-background-light/95 backdrop-blur-md px-4 pt-12 pb-4 flex items-center justify-between border-b border-transparent transition-colors">
                ...
                <h1 className="text-text-main text-lg font-bold tracking-tight absolute left-1/2 transform -translate-x-1/2">Masalah Pesanan</h1>
            ...
            <main className="flex flex-col gap-6 px-4 pt-4">
                ...
                <section>
                    ...
                    <div className="flex flex-col gap-3">
                        <HelpOptionButton label="Pesanan saya belum sampai" onClick={() => onNavigate?.('helpOrderNotArrived')} />
                        <HelpOptionButton label="Item pesanan kurang atau tidak lengkap" onClick={() => onNavigate?.('helpOrderIncomplete')} />
                        <HelpOptionButton label="Makanan tumpah atau kemasan rusak" onClick={() => onNavigate?.('helpOrderDamaged')} />
                        <HelpOptionButton label="Pesanan tidak sesuai dengan aplikasi" onClick={() => onNavigate?.('helpOrderIncorrect')} />
                        <HelpOptionButton label="Driver menyelesaikan pesanan tapi barang belum diterima" onClick={() => onNavigate?.('helpOrderNotReceived')} />
                        <HelpOptionButton label="Saya ingin membatalkan pesanan" onClick={() => onNavigate?.('helpCancelOrder')} />
                        <HelpOptionButton label="Driver meminta uang parkir atau biaya tambahan" onClick={() => onNavigate?.('helpDriverExpenses')} />
                    </div>
                </section>
