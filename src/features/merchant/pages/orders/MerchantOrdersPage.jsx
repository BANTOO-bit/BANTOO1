...
import OrderCardSkeleton from '@/features/shared/components/OrderCardSkeleton'
...
function MerchantOrdersPage() {
    ...
    useRealtimeMulti(
        user?.merchantId ? `merchant-orders-${user.merchantId}` : null,
        [
            {
                table: 'orders',
                event: 'INSERT',
                filter: user?.merchantId ? `merchant_id=eq.${user.merchantId}` : undefined,
                callback: (payload) => {
