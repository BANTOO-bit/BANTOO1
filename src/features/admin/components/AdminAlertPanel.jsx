...
export default function AdminAlertPanel() {
    ...
    const fetchAlerts = async () => {
        try {
            const { codOrders, driverNames, stuckOrders, pendingWithdrawals } = await adminService.getAlertPanelData()

            const alertItems = []

            // 1. Drivers with COD balance not deposited
                ...
                Object.entries(driverTotals).forEach(([driverId, info]) => {
                    if (info.total > 100000) {
                        alertItems.push({
                            ...
                            subtitle: driverNames[driverId] || 'Driver',
            ...
            // 2. Orders stuck in non-terminal status for too long
