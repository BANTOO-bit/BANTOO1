...
export default function AdminOrdersPage() {
    ...
    const fetchOrders = useCallback(async () => {
        try {
            ...
            setStats({
                total: data.length,
                active: data.filter(o => ['pending', 'accepted', 'preparing', 'processing', 'ready', 'pickup', 'picked_up', 'delivering'].includes(o.status)).length,
    ...
    useRealtime('admin-orders-realtime', {
        table: 'orders',
        event: '*',
    }, () => {
        fetchOrders()
    ...
    useEffect(() => {
        ...
        adminService.getCustomerName(customerFilter)
    ...
    return (
        <AdminLayout title={customerFilter ? `Pesanan ${customerName}` : 'Daftar Semua Transaksi'} showBack onBackClick={() => navigate(-1)}>
