...
export default function AdminDriversPage() {
    ...
    const handleTerminate = async () => {
        ...
        try {
            await adminService.terminateDriver(selectedDriver.id, terminateReason)
    ...
    const handleSuspend = async () => {
        if (!selectedDriver) return
        try {
            await adminService.suspendDriver(selectedDriver.id)
    ...
    const handleUnsuspendClick = (driver) => {
        setSelectedDriver(driver)
    ...
    const handleUnsuspend = async () => {
        if (!selectedDriver) return
        try {
            await adminService.unsuspendDriver(selectedDriver.id)
            setDrivers(drivers.map(d => d.id === selectedDriver.id ? { ...d, status: 'approved' } : d))
            fetchStats()
            addToast(`${driverDisplayName(selectedDriver)} telah diaktifkan kembali`, 'success')
        } catch {
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <AdminActionMenu
                                                        items={[
                                                            ...
                                                            { separator: true },
                                                            ...(driver.status === 'suspended'
                                                                ? [{ icon: 'check_circle', label: 'Aktifkan Kembali', onClick: () => handleUnsuspendClick(driver) }]
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#111418]/60 backdrop-blur-sm">
                    <div className="w-full max-w-[440px] bg-white dark:bg-[#1a2632] rounded-xl border border-[#e5e7eb] dark:border-[#2a3b4d]">
                        <div className="p-8">
                            ...
                            <h3 className="text-xl font-bold text-[#111418] dark:text-white mb-3 text-center">Aktifkan Kembali Driver?</h3>
                            <p className="text-[#617589] dark:text-[#94a3b8] text-center mb-8 text-sm leading-relaxed">
                                <strong className="text-[#111418] dark:text-white">{selectedDriver ? driverDisplayName(selectedDriver) : ''}</strong> akan diaktifkan kembali dan bisa menerima order seperti biasa.
                            </p>
                            <div className="grid grid-cols-2 gap-4">
                                <button
