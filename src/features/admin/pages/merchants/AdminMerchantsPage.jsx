...
import { SkeletonCard, SkeletonTable } from '@/features/admin/components/AdminSkeleton'
...
export default function AdminMerchantsPage() {
    ...
    const handleTerminate = async () => {
        ...
        try {
            await adminService.terminateMerchant(selectedMerchant.id, terminateReason)
    ...
    const handleSuspend = async () => {
        if (!selectedMerchant) return
        try {
            await adminService.suspendMerchant(selectedMerchant.id)
    ...
    const handleUnsuspendClick = (merchant) => {
        setSelectedMerchant(merchant)
    ...
    const handleUnsuspend = async () => {
        if (!selectedMerchant) return
        try {
            await adminService.unsuspendMerchant(selectedMerchant.id)
            setMerchants(merchants.map(m => m.id === selectedMerchant.id ? { ...m, status: 'approved' } : m))
            fetchStats()
            addToast(`${selectedMerchant.name} telah diaktifkan kembali`, 'success')
        } catch {
        ...
        <AdminLayout title="Manajemen Warung">
            ...
            {stats.pending > 0 && (
                <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <Link to="/admin/merchants/verification" className="whitespace-nowrap flex items-center gap-2 px-6 py-3 bg-primary hover:bg-blue-700 text-white font-bold rounded-lg transition-colors">
                        Tinjau Antrean
                ...
                <div className="bg-white dark:bg-[#1a2632] border border-[#e5e7eb] dark:border-[#2a3b4d] rounded-xl overflow-hidden">
                    <div className="px-6 py-5 border-b border-[#e5e7eb] dark:border-[#2a3b4d] flex items-center justify-between">
                        ...
                        <div className="relative w-64">
                            ...
                            <input
                                className="w-full pl-9 pr-4 py-2 text-sm bg-[#f0f2f4] dark:bg-[#2a3b4d] border border-[#e5e7eb] dark:border-[#2a3b4d] rounded-lg focus:ring-1 focus:ring-primary focus:border-primary text-[#111418] dark:text-white placeholder:text-[#617589]"
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-[#e5e7eb] dark:divide-[#2a3b4d]">
                                {merchants
                                    .filter(m => !searchQuery || m.name?.toLowerCase().includes(searchQuery.toLowerCase()) || m.profiles?.full_name?.toLowerCase().includes(searchQuery.toLowerCase()))
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <AdminActionMenu
                                                    items={[
                                                        ...
                                                        { separator: true },
                                                        ...(merchant.status === 'suspended'
                                                            ? [{ icon: 'check_circle', label: 'Aktifkan Kembali', onClick: () => handleUnsuspendClick(merchant) }]
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#111418]/60 backdrop-blur-sm">
                    <div className="w-full max-w-[440px] bg-white dark:bg-[#1a2632] rounded-xl border border-[#e5e7eb] dark:border-[#2a3b4d]">
                        <div className="p-8">
                            ...
                            <h3 className="text-xl font-bold text-[#111418] dark:text-white mb-3 text-center">Aktifkan Kembali Warung?</h3>
                            <p className="text-[#617589] dark:text-[#94a3b8] text-center mb-8 text-sm leading-relaxed">
                                <strong className="text-[#111418] dark:text-white">{selectedMerchant?.name}</strong> akan diaktifkan kembali dan bisa menerima pesanan seperti biasa.
                            </p>
                            <div className="grid grid-cols-2 gap-4">
                                <button
