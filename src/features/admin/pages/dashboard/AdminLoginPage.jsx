...
export default function AdminLoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Check if user has admin role via service
            const isAdmin = await adminService.checkAdminRole(data.user.id)
        ...
        <div className="min-h-screen bg-[#0f1923] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md">
                ...
                <div className="bg-[#1a2632] border border-[#2a3b4d] rounded-2xl p-8 shadow-2xl">
                    ...
                    {sessionMessage && (
                        <div className="mb-6 p-3 bg-amber-900/20 border border-amber-800/30 text-amber-400 rounded-xl text-sm flex items-start gap-2">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full h-12 bg-primary hover:bg-blue-700 text-white font-bold rounded-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
                        >
                            {loading ? (
                                <ButtonLoader />
                            ) : (
