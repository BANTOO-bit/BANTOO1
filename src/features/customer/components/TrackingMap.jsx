...
// Helper to auto-fit bounds (runs on initial load)
...
/**
 * Auto-pan the map to follow the driver when actively moving.
 * Only pans when the driver marker is outside the current viewport.
 */
function AutoPanToDriver({ driverPosition }) {
    const map = useMap()

    useEffect(() => {
        if (!driverPosition) return
        const driverLatLng = L.latLng(driverPosition)
        // Only pan if driver is outside the visible area
        if (!map.getBounds().contains(driverLatLng)) {
            map.panTo(driverLatLng, { animate: true, duration: 1 })
        }
    }, [map, driverPosition])
            ...
            >
                ...
                {driverLocation && (
                    <>
                        ...
                        <AutoPanToDriver driverPosition={driverLocation} />
                    </>
                )}
