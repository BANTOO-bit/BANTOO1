        ...
        <div className="relative min-h-screen flex flex-col bg-background-light pb-6">
            ...
            <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md px-4 pt-12 pb-3 border-b border-border-color">
                <div className="relative flex items-center justify-center min-h-[40px]">
                    ...
                    <h1 className="text-text-main text-lg font-bold tracking-tight">Masalah Pesanan</h1>
            ...
            <main className="flex flex-col gap-6 px-4 pt-4">
                {/* FAQ List */}
                <section>
                    <h2 className="text-sm font-bold text-text-main mb-3 px-1">Pertanyaan Umum</h2>
                    <div className="flex flex-col gap-2">
                        <HelpOptionButton label="Pesanan saya belum sampai" onClick={() => navigate('/help/order/not-arrived')} />
                        <HelpOptionButton label="Item pesanan kurang atau tidak lengkap" onClick={() => navigate('/help/order/incomplete')} />
                        <HelpOptionButton label="Makanan tumpah atau kemasan rusak" onClick={() => navigate('/help/order/damaged')} />
                        <HelpOptionButton label="Pesanan tidak sesuai dengan aplikasi" onClick={() => navigate('/help/order/incorrect')} />
                        <HelpOptionButton label="Driver menyelesaikan pesanan tapi barang belum diterima" onClick={() => navigate('/help/order/not-received')} />
                        <HelpOptionButton label="Saya ingin membatalkan pesanan" onClick={() => navigate('/help/order/cancel')} />
                        <HelpOptionButton label="Driver meminta uang parkir atau biaya tambahan" onClick={() => navigate('/help/order/driver-expenses')} />
                    </div>
                </section>
