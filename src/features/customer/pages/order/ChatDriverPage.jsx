...
function ChatDriverPage() {
    ...
    useEffect(() => {
        ...
        const fetchOrderInfo = async () => {
            try {
                const data = await orderService.getOrderDriverInfo(orderId)
                ...
                if (data?.driver_profile) {
                    ...
                    if (data.driver_detail) {
                        driverData.vehicle = [data.driver_detail.vehicle_brand, data.driver_detail.vehicle_type].filter(Boolean).join(' ') || 'Motor'
                        driverData.plate = data.driver_detail.vehicle_plate || '-'
                    }
