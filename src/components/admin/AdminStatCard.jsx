    ...
    return (
        <div className="bg-white dark:bg-[#1a2632] p-4 rounded-xl border border-[#e5e7eb] dark:border-[#2a3b4d] shadow-sm flex flex-col gap-2 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
                <p className="text-[#617589] dark:text-[#94a3b8] text-sm font-medium">{title}</p>
                <div className={`${colors.bg} ${colors.text} p-1.5 rounded-lg`}>
                    <span className="material-symbols-outlined text-[20px]">{icon}</span>
                </div>
            </div>
            <div className="flex items-end justify-between relative z-10">
                <p className="text-2xl font-bold text-[#111418] dark:text-white tracking-tight">{value}</p>
                {trend ? (
                    ...
                    <span className={`text-sm font-medium ${color === 'red' ? 'text-red-600 dark:text-red-400 animate-pulse' : 'text-[#617589] dark:text-[#94a3b8]'}`}>
                        {subtext}
                    </span>
                )}
            </div>
