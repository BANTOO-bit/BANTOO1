...
function BottomNavigation({ activeTab }) {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-card-dark border-t border-border-color dark:border-gray-800 pb-safe pt-2 z-50">
            <div className="flex justify-around items-center h-16 pb-2">
                {navItems.map((item) => (
                    ...
                    >
                        <span className={`material-symbols-outlined ${effectiveActiveTab === item.id ? 'fill' : ''}`}>
                            {item.icon}
                        </span>
